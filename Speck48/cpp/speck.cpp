...
bool check_testvector() {
    // Verify Speck48_72
    #ifdef SPECK48_72
    printf("Test Speck48-72...\n");
    word mk[M] = {0x121110u, 0x0a0908u, 0x020100u};
    block p = {0x20796cu, 0x6c6172u};
    word keys[MAX_NR];
    block c;
    expand_key(mk, keys, MAX_NR);
    encrypt(p, keys, MAX_NR, c);
    if (c == block(0xc049a5u, 0x385adcu)) {
    ...
    // Verify Speck48_96
    #ifdef SPECK48_96
    printf("Test Speck48-96...\n");
    word mk[M] = {0x1a1918u, 0x121110u, 0x0a0908u, 0x020100u};
    block p = {0x6d2073u, 0x696874u};
    word keys[MAX_NR];
    block c;
    expand_key(mk, keys, MAX_NR);
    encrypt(p, keys, MAX_NR, c);
    if (c == block(0x735e10u, 0xb6445du)) {
        printf("Testvector verified.\n");
    } else {
        printf("Testvector not verified.\n");
        return false;
