...
def calculate_back(c):
    c0, c1, c2, c3 = c
    return (c3, ror(c0, 9), rol(c1, 5), rol(c2, 3))

def make_train_data(n, nr, diff, key_bit_length=128, data_format="x_x\'", calc_back=False):
    ...
    if calc_back:
        cl0, cl1, cl2, cl3 = calculate_back((cl0, cl1, cl2, cl3))
        cr0, cr1, cr2, cr3 = calculate_back((cr0, cr1, cr2, cr3))
    if data_format == "dx_x":
        X = convert_to_binary([cl0 ^ cr0, cl1 ^ cr1, cl2 ^ cr2, cl3 ^ cr3, cl0, cl1, cl2, cl3])
    elif data_format == "x_x\'":
...
def check_testvectors(version=128):
    ...
    if version == 128:
        ...
        key_bit_length = 128
        nr = 24
        key = np.array([[0x3c2d1e0f], [0x78695a4b], [0xb4a59687], [0xf0e1d2c3]], dtype=np.uint32)
        plaintext = np.array([[0x13121110], [0x17161514], [0x1b1a1918], [0x1f1e1d1c]], dtype=np.uint32)
        ciphertext = np.array([[0x354ec89f], [0x18c6c628], [0xa7c73255], [0xfd8b6404]], dtype=np.uint32)

    if version == 192:
        # test_vector for LEA-192
        key_bit_length = 192
        nr = 28
        key = np.array([[0x53af3714], [0x75bd6930], [0x0c56c125], [0xa1d2ba78], [0x1c6734e5], [0x7cf27e00]], dtype=np.uint32)
        plaintext = np.array([[0xcbf4b41c], [0x51db4b6c], [0x0984ea68], [0x51fd7b72]], dtype=np.uint32)
        ciphertext = np.array([[0x6d5c7269], [0xb7f812f9], [0xe611b50e], [0x70583c66]], dtype=np.uint32)

    if version == 256:
        # test_vector for LEA-256
        key_bit_length = 256
        nr = 32
        key = np.array([[0xe279674f], [0x19931ebd], [0xac1530c6], [0xa7d7efff], [0x59edf091], [0x07701bdf], [0xe282fe69], [0x358c66f0]], dtype=np.uint32)
        plaintext = np.array([[0xe3ca31dc], [0x110a5eda], [0x20b066c9], [0xdefecfd7]], dtype=np.uint32)
        ciphertext = np.array([[0x2004a2ed], [0xe867f698], [0xb82da057], [0xf2dfa7ca]], dtype=np.uint32)

    ks = expand_key(key, nr=nr, key_bit_length=key_bit_length)

    c = encrypt(plaintext, ks)
    
    if np.sum(np.array(c, dtype=np.uint32) == ciphertext) == 4:
        if key_bit_length == 128:
