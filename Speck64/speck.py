...
def check_testvector():
    ...
    n = 10**6
    pt = np.frombuffer(urandom(8 * n), dtype=np.uint32).reshape(2, n)
    key = np.frombuffer(urandom(12 * n), dtype=np.uint32).reshape(3, n)
    ks = expand_key(key, 26)
    ct = encrypt(pt, ks)
    pt_tmp = decrypt(ct, ks)
    if np.sum(pt_tmp[0] == pt[0]) == n and np.sum(pt_tmp[1] == pt[1]) == n:
...
def make_train_data_only_diff(n, nr, diff, master_key_bit_length=96):
    assert master_key_bit_length % WORD_SIZE() == 0
    m = master_key_bit_length // WORD_SIZE()
    assert m == 3 or m == 4
    Y = np.frombuffer(urandom(n), dtype=np.uint8) & 1
...
def make_tar_diff_samples(n, nr, diff, common_key=False, given_ks=None, master_key_bit_length=96):
    assert master_key_bit_length % WORD_SIZE() == 0
    m = master_key_bit_length // WORD_SIZE()
    assert m == 3 or m == 4
    if not common_key:
        ...
        keys = np.frombuffer(urandom(4 * m), dtype=np.uint32).reshape(m, 1)
    if given_ks is None:
        ks = expand_key(keys, nr)
    else:
        ks = given_ks
